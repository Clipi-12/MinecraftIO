...
import me.clipi.io.nbt.exceptions.NbtDuplicatedKeyException;
import me.clipi.io.nbt.exceptions.NbtKeyNotFoundException;
import me.clipi.io.nbt.exceptions.NbtTypeParseException;
 ...
 */
public class NbtCompound implements NestedToString {
	private final @NotNull GrowableArray<@NotNull String[]> keys = GrowableArray.generic(String.class);
	private final @NotNull GrowableArray<byte[]> types = GrowableArray.bytes();
	...
	private void addKey(@NotNull String key, byte nbtType) throws NbtDuplicatedKeyException,
		...
		for (--len; len >= 0; --len) {
			if (key.equals(keys[len])) throw new NbtDuplicatedKeyException(key, this);
		}
	...
	void addByte(@NotNull String key, byte value) throws NbtDuplicatedKeyException,
														 CheckedBigEndianDataInput.OomException {
		addKey(key, NbtType.tagByte);
		GrowableArray.add(bytes == null ? bytes = GrowableArray.bytes() : bytes, value);
	}

	void addShort(@NotNull String key, short value) throws NbtDuplicatedKeyException,
														   CheckedBigEndianDataInput.OomException {
		addKey(key, NbtType.tagShort);
		GrowableArray.add(shorts == null ? shorts = GrowableArray.shorts() : shorts, value);
	}

	void addInt(@NotNull String key, int value) throws NbtDuplicatedKeyException,
													   CheckedBigEndianDataInput.OomException {
		addKey(key, NbtType.tagInt);
		GrowableArray.add(ints == null ? ints = GrowableArray.ints() : ints, value);
	}

	void addLong(@NotNull String key, long value) throws NbtDuplicatedKeyException,
														 CheckedBigEndianDataInput.OomException {
		addKey(key, NbtType.tagLong);
		GrowableArray.add(longs == null ? longs = GrowableArray.longs() : longs, value);
	}

	void addFloat(@NotNull String key, float value) throws NbtDuplicatedKeyException,
														   CheckedBigEndianDataInput.OomException {
		addKey(key, NbtType.tagFloat);
		GrowableArray.add(floats == null ? floats = GrowableArray.floats() : floats, value);
	}

	void addDouble(@NotNull String key, double value) throws NbtDuplicatedKeyException,
															 CheckedBigEndianDataInput.OomException {
		addKey(key, NbtType.tagDouble);
		GrowableArray.add(doubles == null ? doubles = GrowableArray.doubles() : doubles, value);
	}

	private void addObject(@NotNull String key, @NotNull Object value, byte nbtType) throws NbtDuplicatedKeyException,
																							CheckedBigEndianDataInput.OomException {
		addKey(key, nbtType);
		GrowableArray.add(objects == null ? objects = GrowableArray.generic(Object.class) : objects, value);
	}

	void addByteArray(@NotNull String key, byte @NotNull [] value) throws NbtDuplicatedKeyException,
																		  CheckedBigEndianDataInput.OomException {
		addObject(key, value, NbtType.tagByteArray);
	}

	void addIntArray(@NotNull String key, int @NotNull [] value) throws NbtDuplicatedKeyException,
																		CheckedBigEndianDataInput.OomException {
		addObject(key, value, NbtType.tagIntArray);
	}

	void addLongArray(@NotNull String key, long @NotNull [] value) throws NbtDuplicatedKeyException,
																		  CheckedBigEndianDataInput.OomException {
		addObject(key, value, NbtType.tagLongArray);
	}

	void addString(@NotNull String key, @NotNull String value) throws NbtDuplicatedKeyException,
																	  CheckedBigEndianDataInput.OomException {
		addObject(key, value, NbtType.tagString);
	}

	void addList(@NotNull String key, @NotNull NbtList value) throws NbtDuplicatedKeyException,
																	 CheckedBigEndianDataInput.OomException {
		addObject(key, value, NbtType.tagList);
	}

	void addMap(@NotNull String key, @NotNull NbtCompound value) throws NbtDuplicatedKeyException,
																		CheckedBigEndianDataInput.OomException {
	...
	private int indexForKeyWithType(@NotNull String key, byte nbtType) throws NbtTypeParseException,
		...
		for (int i = 0; i < len; ++i) {
			if (key.equals(keys[i])) {
				if (types[i] != nbtType) throw new NbtTypeParseException(NbtType.values()[nbtType], types[i]);
				return count;
			}
	...
	public byte getByte(@NotNull String key) throws NbtTypeParseException, NbtKeyNotFoundException {
		assert bytes != null;
		return bytes.inner[indexForKeyWithType(key, NbtType.tagByte)];
	}

	public short getShort(@NotNull String key) throws NbtTypeParseException, NbtKeyNotFoundException {
		assert shorts != null;
		return shorts.inner[indexForKeyWithType(key, NbtType.tagShort)];
	}

	public int getInt(@NotNull String key) throws NbtTypeParseException, NbtKeyNotFoundException {
		assert ints != null;
		return ints.inner[indexForKeyWithType(key, NbtType.tagInt)];
	}

	public long getLong(@NotNull String key) throws NbtTypeParseException, NbtKeyNotFoundException {
		assert longs != null;
		return longs.inner[indexForKeyWithType(key, NbtType.tagLong)];
	}

	public float getFloat(@NotNull String key) throws NbtTypeParseException, NbtKeyNotFoundException {
		assert floats != null;
		return floats.inner[indexForKeyWithType(key, NbtType.tagFloat)];
	}

	public double getDouble(@NotNull String key) throws NbtTypeParseException, NbtKeyNotFoundException {
		assert bytes != null;
		return bytes.inner[indexForKeyWithType(key, NbtType.tagDouble)];
	}

	public byte @NotNull [] getByteArray(@NotNull String key) throws NbtTypeParseException, NbtKeyNotFoundException {
		assert objects != null;
		return (byte @NotNull []) objects.inner[indexForKeyWithType(key, NbtType.tagByteArray)];
	}

	public int @NotNull [] getIntArray(@NotNull String key) throws NbtTypeParseException, NbtKeyNotFoundException {
		assert objects != null;
		return (int @NotNull []) objects.inner[indexForKeyWithType(key, NbtType.tagIntArray)];
	}

	public long @NotNull [] getLongArray(@NotNull String key) throws NbtTypeParseException, NbtKeyNotFoundException {
		assert objects != null;
		return (long @NotNull []) objects.inner[indexForKeyWithType(key, NbtType.tagLongArray)];
	}

	public @NotNull String getString(@NotNull String key) throws NbtTypeParseException, NbtKeyNotFoundException {
		assert objects != null;
		return (@NotNull String) objects.inner[indexForKeyWithType(key, NbtType.tagString)];
	}

	public @NotNull NbtList getList(@NotNull String key) throws NbtTypeParseException, NbtKeyNotFoundException {
		assert objects != null;
		return (@NotNull NbtList) objects.inner[indexForKeyWithType(key, NbtType.tagList)];
	}

	public @NotNull NbtCompound getMap(@NotNull String key) throws NbtTypeParseException, NbtKeyNotFoundException {
		assert objects != null;
